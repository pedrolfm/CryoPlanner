...
class CryoSim(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.title = "CryoSim" # TODO make this more human readable by adding spaces
    self.parent.categories = ["Examples"]
    self.parent.dependencies = []
    self.parent.contributors = ["Pedro Moreira (BWH)"] # replace with "Firstname Lastname (Organization)"
    self.parent.helpText = """
This a module to compute the best cryo-probes in mri-guided focal ablation of the prostate
"""
...
class CryoSimWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    self.inputT2image.setToolTip( "Pick the input to the algorithm." )
...
class CryoSimLogic(ScriptedLoadableModuleLogic):
...
class Cryo2Probes(ScriptedLoadableModuleLogic):
  ...
  def TestFunction(self, inputVolume, inputVolume2, inputVolume3, inputVolume4, StdevError, NoOfProbes, Kd, N, outputLabel):
    ...
    resultFileName = "Results-N%02d-K%02d-2probes-new-opt-r38" % (N, self.DiceGain)
    resultFilePath = '/Users/pedro/Documents/Test-slicer' + '/' + resultFileName
    resultFile = open(resultFilePath, 'a')
...
#
  ...
  def optimizationCryo(self, N, Kd):
    ...
    def Fun(x):
        ...
        temp = Metric[0] / float(N)
        # ATENCAO AQUI!!!!
        return -(temp+1.0) * Metric[1] #-temp * temp * Metric[1]
